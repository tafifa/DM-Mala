...
def get_color_range(dir):
    ...
    # Prompt the user to enter the lower color range
    print("Enter the lower color range (in RGB format)")
    lower_rgb = tuple(int(x) for x in input("R, G, B: ").split(','))

    # Prompt the user to enter the upper color range
    print("Enter the upper color range (in RGB format)")
    upper_rgb = tuple(int(x) for x in input("R, G, B: ").split(','))

    # Convert the RGB color range to the HSV color range
    ...
    # Create a mask to threshold the image within the specified color range
    mask = cv2.inRange(hsv_image, lower_hsv, upper_hsv)

    # Apply the mask to the original image
    color_range_image = cv2.bitwise_and(image, image, mask=mask)

    # Display the color range image
    cv2.imshow('Color Range Image', color_range_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
